<x-layouts.app>
    <div class="min-h-[80vh] flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 cyber-glass p-8 border-t-2 border-cyan-600 dark:border-cyan-500 animate-reveal transition-all duration-300">
            <!-- Header -->
            <div class="text-center">
                <h2 class="font-display text-3xl font-black tracking-tighter text-slate-900 dark:text-white uppercase">
                    Access <span class="text-cyan-600 dark:text-cyan-400">Granted</span>
                </h2>
            ...
            <!-- Session Status -->
            @if (session('status'))
                <div class="bg-cyan-50 dark:bg-cyan-500/10 border border-cyan-200 dark:border-cyan-500/50 p-3 text-cyan-700 dark:text-cyan-400 text-xs font-mono">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form class="mt-8 space-y-6" action="{{ route('login') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div class="form-control">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus
                            class="input input-bordered w-full bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-white/10 text-cyan-700 dark:text-cyan-400 focus:border-cyan-600 dark:focus:border-cyan-500 focus:ring-1 focus:ring-cyan-600 dark:focus:ring-cyan-500 transition-all font-mono"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <input id="remember_me" name="remember" type="checkbox" class="checkbox checkbox-xs checkbox-primary border-slate-300 dark:border-white/20">
                        <label for="remember_me" class="ml-2 block text-xs text-slate-500 dark:text-slate-400 font-display uppercase tracking-wider font-bold">
                            Remember session
                        </label>
                    </div>
                    ...
                    @if (Route::has('password.request'))
                        <div class="text-xs">
                            <a href="{{ route('password.request') }}" class="font-display uppercase tracking-wider text-cyan-600 dark:text-cyan-500 hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors font-bold">
                                Lost key?
                            </a>
                        </div>
                    @endif
