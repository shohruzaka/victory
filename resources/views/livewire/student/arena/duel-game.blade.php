            ...
            <div class="cyber-glass p-8 md:p-12 space-y-12 animate-reveal transition-all duration-300" wire:key="question-{{ $currentIndex }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($currentOptions as $index => $option)
                        ...
                        <button 
                            ...
                            {{ $showResult ? 'disabled' : '' }}
                        >
                            <span class="font-mono text-xs opacity-40" wire:loading.class="opacity-50">{{ chr(65 + $index) }}.</span>
                            <span class="font-mono text-xs md:text-sm flex-grow" wire:loading.class="opacity-50">{{ $option['text'] }}</span>
                            <span wire:loading wire:target="answer({{ $option['id'] }})">
                ...
                @if($showResult)
                    <div class="pt-8 border-t border-slate-100 dark:border-white/5 flex justify-end">
                        <button wire:click="nextQuestion" wire:loading.attr="disabled" class="btn btn-outline border-cyan-600 dark:border-cyan-500 text-cyan-700 dark:text-cyan-400 hover:bg-cyan-600 hover:text-white dark:hover:bg-cyan-500 dark:hover:text-slate-950 rounded-none font-display uppercase tracking-widest text-xs px-8 transition-all min-w-[160px] flex items-center justify-center">
